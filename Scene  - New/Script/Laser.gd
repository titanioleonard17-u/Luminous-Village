...
@export var laser_color: Color = Color(1, 0.1, 0.1)
@export var laser_width: float = 3.0
@export var start_direction: Vector2 = Vector2.RIGHT

@onready var bounceCounter: CanvasLayer = $"../MirrorCounter"

# --- Pengaturan glow ---
@export_range(0, 25, 1) var glow_layers: int = 20
@export_range(0.5, 15.0, 0.1) var glow_width_multiplier: float = 10.0
@export_range(0.0, 1.0, 0.01) var glow_alpha: float = 0.18
@export_range(0.5, 5.0, 0.1) var glow_softness: float = 3.0

# --- Fade sepanjang path ---
@export var fade_along_length: bool = true
@export_range(0.0, 1.0, 0.01) var fade_start_ratio: float = 0.0
@export_range(0.5, 5.0, 0.1) var fade_softness: float = 1.8
...
func _ready() -> void:
	...
	_update_bounce_counter()

func _physics_process(_delta: float) -> void:
	var all_paths: Array = _calculate_laser_paths()
	var highest_bounce: int = 0

	for path_data in all_paths:
		var bounce_count: int = int(path_data["bounce_count"])
		highest_bounce = max(highest_bounce, bounce_count)

	current_bounces = highest_bounce
	_update_bounce_counter()
	queue_redraw()
...
func _draw() -> void:
	...
	var drawn_joints: Dictionary = {}
	var drawn_impacts: Dictionary = {}
...
func _draw_path(points: Array, skip_glow: Array, drawn_segments: Dictionary, drawn_joints: Dictionary, drawn_impacts: Dictionary) -> void:
	...
	for layer in range(glow_layers, 0, -1):
		...
		for i in range(points.size() - 1):
			...
			if not drawn_segments.has(seg_key):
...
func _calculate_laser_paths() -> Array:
	...
	return _trace_ray(start_position, dir, max_bounces, [], 0)

func _trace_ray(start_pos: Vector2, start_dir: Vector2, bounces_left: int, exclude_rids: Array[RID], bounce_count: int) -> Array:
	var points: Array[Vector2] = [start_pos]
	var skip_glow: Array[bool] = [true]
	...
	while remaining > 0:
		...
		if collider.is_in_group("door"):
			...
			for split_dir in split_dirs:
				...
				var sub_paths: Array = _trace_ray(
					branch_start,
					split_dir,
					remaining,
					branch_exclude,
...
func _update_bounce_counter() -> void:
	if bounceCounter:
		bounceCounter.setBounceCount(current_bounces, max_bounces)
